...
async def stream_proxy(request: Request) -> StreamingResponse:
        ...
        try:
            while True:
                ...
                if chunk is None:
                    break
                yield chunk
                # Non-blocking disconnect check after each chunk.
    ...
    return StreamingResponse(
        ...
        headers={
            # No Accept-Ranges — this is a live stream, not a file.
            "Accept-Ranges": "none",
            "Cache-Control": "no-cache, no-store",
            # Keep-alive lets iOS hold the connection open.
            "Connection": "keep-alive",
            "X-Content-Type-Options": "nosniff",
            "Access-Control-Allow-Origin": "*",
            # Minimal ICY headers so players show station name.
...
async def health() -> HealthResponse:
    ...
    # The persistent audio encoder is a core internal component, so a failure
    # while the player is supposed to be streaming is a liveness failure.
    core_healthy = queue_ok and player_ok and api_ok and ffmpeg_ok
